...
// opts (e.g. cwd) are not threaded through the responder; read them from the
// shared mock's recorded call args.
function execOpts(i: number): { cwd?: string } | undefined {
  return mockExecSync.mock.calls[i]?.[1] as { cwd?: string } | undefined;
}
